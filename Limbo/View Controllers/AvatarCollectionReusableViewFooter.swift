...
class AvatarCollectionReusableViewFooter: UICollectionReusableView, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        let request = createHttpRequestForImgur(pickedImage: pickedImage)
        let task = createTaskForImgur(request: request)
        task.resume()
        self.imagePickingDelegate.didStarUploadingImage()
        self.presentingVC?.dismiss(animated: true, completion:nil)
        self.presentingVC?.dismiss(animated: true, completion:nil)
    }
    ...
    private func createHttpRequestForImgur(pickedImage: UIImage) -> URLRequest {
    ...
    private func createTaskForImgur(request: URLRequest) -> URLSessionDataTask {
